  ...
  def findAgentJar(): os.Path = {
    ...
    val candidates =
      if os.exists(agentTarget) then
        os.walk(agentTarget).filter(p => os.isFile(p) && p.last == "lazyvalgrade-agent.jar")
      else Seq.empty
    candidates.headOption.getOrElse(
      throw new RuntimeException(
        s"Agent jar not found under $agentTarget (expected lazyvalgrade-agent.jar; should be built by sbt via agent/assembly)"
      )
    )
  }
